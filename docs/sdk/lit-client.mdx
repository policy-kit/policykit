...
1. **Lit Action deployed to IPFS** — The `policyEvaluator.action.ts` from the `lit-actions` package
2. **PKP (Programmable Key Pair)** — Provisioned through Lit Protocol
3. **Capacity Credits** — To pay for Lit network usage

<Note>
  The `LitClient` requires peer dependencies: `@lit-protocol/lit-node-client`, `@lit-protocol/constants`, and `@lit-protocol/auth-helpers`. Install them separately.
</Note>
